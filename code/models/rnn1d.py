...
class RNN1d(nn.Sequential):
    def __init__(self,input_channels,num_classes,lstm=True,hidden_dim=256,num_layers=2,bidirectional=False,ps_head=0.5,act_head="relu",lin_ftrs_head=None,bn=True):
        ...
        #bs, ch, ts -> ts, bs, ch
        #LSTM
        layers_tmp.append(lambda x: x.transpose(1,2)) # Replaced Lambda with a simple function
        layers_tmp.append(lambda x: x.transpose(0,1)) # Replaced Lambda with a simple function
        if(lstm):
        ...
        #pooling
        layers_tmp.append(lambda x: x[0].transpose(0,1)) # Replaced Lambda with a simple function
        layers_tmp.append(lambda x: x.transpose(1,2)) # Replaced Lambda with a simple function
        ...
        ps_head = [ps_head/2] * (len(lin_ftrs_head)-2) + [ps_head] if isinstance(ps_head, list) else ps_head # Replaced listify with isinstance
        ...
        for ni, no, p, actn in zip(lin_ftrs_head[:-1], lin_ftrs_head[1:], ps_head, actns):
            layers_head.append(nn.BatchNorm1d(ni))
            layers_head.append(nn.Dropout(p))
            layers_head.append(nn.Linear(ni, no))
            if actn is not None:
                layers_head.append(actn)
