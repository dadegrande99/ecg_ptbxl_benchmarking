...
class ResNet1D(BaseModelEE, L.LightningModule):
    def __init__(self, block, layers, num_classes=2, in_channels=12, dropout_rate=0.05, learning_rate=0.1):
        ...
        self.modules_EE.append(self._make_layer(
            block, 64, layers[0], dropout_rate=dropout_rate))
        for i in range(1, len(layers)):
            self.modules_EE.append(self._make_layer(
                block, 64 * 2**i, layers[i], stride=2, dropout_rate=dropout_rate))
        self.avgpool = nn.AdaptiveAvgPool1d(1)
        self.dropout = nn.Dropout(p=dropout_rate)
        self.fc = nn.Linear(64 * 2**(len(layers)-1) *
                            block.expansion, num_classes)

        # Add intermediate classifiers for early exits
        self.exits = nn.ModuleList([nn.Linear(
            64 * 2**i * block.expansion, num_classes) for i in range(len(self.modules_EE))])
    ...
    def _make_layer(self, block, out_channels, blocks, stride=1, dropout_rate=0.05):
        ...
        layers.append(block(self.in_channels, out_channels, stride=stride,
                      downsample=downsample, dropout_rate=dropout_rate))
